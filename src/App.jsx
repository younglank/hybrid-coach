...
const DEFAULT_SETTINGS = { barWeight:45, proteinTarget:160, carbTarget:230, fatTarget:70, waterTarget:120, sleepNeed:9.5, wakeTime:"07:00", name:"", heightFt:5, heightIn:9, weight:170, age:25, sex:"male", activityLevel:"active", experience:"intermediate", goal:"hybrid", onboardingDone:false };
...
function CheckinFlow({ existingDay, onSave, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.92)", zIndex:100, overflowY:"auto" }}>
      <div className="fix-safe-top" style={{ maxWidth:"480px", margin:"0 auto", background:"#0a0a0a", minHeight:"100vh" }}>
        <div className="safe-sticky" style={{ padding:"16px 20px", borderBottom:"1px solid #2a2a2a", display:"flex", justifyContent:"space-between", alignItems:"center", position:"sticky", top:0, background:"#0a0a0a", zIndex:2 }}>
...
function PresetEditor({ presets, onSave, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
      <div className="fix-safe-bottom" style={{ maxWidth:"480px", width:"100%", background:"#0a0a0a", borderTop:"1px solid #333", borderRadius:"12px 12px 0 0", padding:"20px", maxHeight:"85vh", overflowY:"auto", animation:"sheet .25s ease both" }}>
...
function SettingsSheet({ settings, onSave, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:300, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
      <div className="fix-safe-bottom" style={{ maxWidth:"480px", width:"100%", background:"#0a0a0a", borderTop:"1px solid #333", borderRadius:"12px 12px 0 0", padding:"20px", maxHeight:"88vh", overflowY:"auto", animation:"sheet .25s ease both" }}>
...
function DayView({ day, isToday, onCheckin, onUpdate, prevDays, streak, totals, volume, presets, onSavePresets, bumpFreq, buckets, prs, settings }) {
  ...
  return (
    <>
      ...
      <div className="fix-safe-bottom" style={{ position:"fixed", bottom:0, left:0, right:0, maxWidth:"480px", margin:"0 auto", padding:"12px 16px", background:"linear-gradient(transparent, #0a0a0a 30%)", zIndex:50 }}>
      ...
      {showCoach && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
          <div className="fix-safe-bottom" style={{ maxWidth:"480px", width:"100%", background:"#0a0a0a", borderTop:"1px solid #333", borderRadius:"12px 12px 0 0", padding:"18px 18px 22px", maxHeight:"82vh", display:"flex", flexDirection:"column", animation:"sheet .25s ease both" }}>
...
// Mifflin-St Jeor (Frankenfield et al. 2005 — most validated BMR equation)
function calcBMR(weightLb, heightIn, age, sex) {
  const kg = weightLb * 0.453592, cm = heightIn * 2.54;
  return Math.round(sex === "female" ? (10*kg + 6.25*cm - 5*age - 161) : (10*kg + 6.25*cm - 5*age + 5));
}
function calcTDEE(bmr, level) {
  return Math.round(bmr * ({ sedentary:1.2, light:1.375, moderate:1.55, active:1.725, very_active:1.9 }[level] || 1.55));
}
// Protein per research: 1.6-2.2g/kg optimal for hypertrophy (Morton et al. 2018, Br J Sports Med)
// Fat ≥20% for hormonal health (Volek et al.); carbs = remainder (fuel training)
function calcMacros(tdee, weightLb, goal) {
  let cal = tdee;
  if (goal==="muscle") cal += 350;       // lean surplus
  else if (goal==="cut") cal -= 500;     // ~1lb/wk deficit
  else if (goal==="recomp") cal -= 100;  // slight deficit, high protein
  else if (goal==="endurance") cal += 200; // fuel volume
  const ppl = ({ muscle:1.0, cut:1.1, recomp:1.0, hybrid:1.0, endurance:0.8 })[goal] || 0.9;
  const protein = Math.round(weightLb * ppl);
  ...
  return { cal:actualCal, protein, carbs, fat, ppl };
}

// ── FIRST-RUN ONBOARDING ─────────────────────────────────────────────────────
function OnboardingFlow({ settings, onComplete }) {
  const [step, setStep] = useState(1);
  const [d, setD] = useState({
    name:settings.name||"", heightFt:settings.heightFt||"5", heightIn:settings.heightIn||"9",
    weight:settings.weight||"170", age:settings.age||"25", sex:settings.sex||"male",
    activityLevel:settings.activityLevel||"active", experience:settings.experience||"intermediate",
    goal:settings.goal||"hybrid", barWeight:settings.barWeight||"45",
    sleepNeed:settings.sleepNeed||"9.5", wakeTime:settings.wakeTime||"07:00",
  });
  const set = (k,v) => setD(p=>({...p,[k]:v}));
  const totalIn = (parseInt(d.heightFt)||0)*12 + (parseInt(d.heightIn)||0);
  const wt = parseFloat(d.weight)||150;
  const bmr = calcBMR(wt, totalIn, parseInt(d.age)||25, d.sex);
  const tdee = calcTDEE(bmr, d.activityLevel);
  const m = calcMacros(tdee, wt, d.goal);
  const waterRec = Math.round(wt * 0.5);
  const [targets, setTargets] = useState(null);
  useEffect(()=>{ if(step===5) setTargets({ protein:m.protein, carbs:m.carbs, fat:m.fat, water:waterRec, cal:m.cal }); }, [step]);
  const t = targets || { protein:m.protein, carbs:m.carbs, fat:m.fat, water:waterRec, cal:m.cal };
  const tSet = (k,v) => setTargets(p=>({...p,[k]:v}));

  function finish() {
    onComplete({
      ...settings, name:d.name.trim(), heightFt:parseInt(d.heightFt)||5, heightIn:parseInt(d.heightIn)||9,
      weight:parseFloat(d.weight)||170, age:parseInt(d.age)||25, sex:d.sex,
      activityLevel:d.activityLevel, experience:d.experience, goal:d.goal,
      barWeight:parseFloat(d.barWeight)||45, sleepNeed:parseFloat(d.sleepNeed)||9.5, wakeTime:d.wakeTime||"07:00",
      proteinTarget:parseInt(t.protein)||m.protein, carbTarget:parseInt(t.carbs)||m.carbs,
      fatTarget:parseInt(t.fat)||m.fat, waterTarget:parseInt(t.water)||waterRec, onboardingDone:true,
    });
  }

  const chip = (val, cur, fn, accent="#E8FF47") => (
    <button onClick={()=>fn(val)} style={{ padding:"10px 14px", borderRadius:"3px", fontSize:"12px", letterSpacing:"1px",
      cursor:"pointer", fontFamily:"inherit", fontWeight:cur===val?"700":"500",
      border:cur===val?`1.5px solid ${accent}`:"1.5px solid #3a3a3a",
      background:cur===val?accent:"#111", color:cur===val?"#0a0a0a":"#bbb" }}>{val}</button>
  );
  const goalLabel = { muscle:"Build Muscle", cut:"Lose Fat", recomp:"Recomp", endurance:"Endurance", hybrid:"Hybrid Athlete" };
  const actLabel = { sedentary:"Sedentary", light:"Light (1-3d)", moderate:"Moderate (3-5d)", active:"Active (6-7d)", very_active:"Very Active" };
  const expLabel = { beginner:"Beginner (<1yr)", intermediate:"Intermediate (1-3yr)", advanced:"Advanced (3+yr)" };
  const dots = Array.from({length:5},(_,i)=>i+1);

  return (
    <div style={{ padding:"24px 20px", maxWidth:"480px", margin:"0 auto" }}>
      <div style={{ display:"flex", justifyContent:"center", gap:"8px", marginBottom:"28px" }}>
        {dots.map(i=>(<div key={i} style={{ width:i===step?"24px":"8px", height:"8px", borderRadius:"4px", background:i<=step?"#E8FF47":"#2a2a2a", transition:"all .3s" }} />))}
      </div>

      {step===1 && (<div>
        <div style={{ fontSize:"28px", fontWeight:"700", lineHeight:1.2, marginBottom:"6px" }}>LET'S BUILD<br/><span style={{ color:"#E8FF47" }}>YOUR COACH</span></div>
        <div style={{ fontSize:"13px", color:"#9a9a9a", lineHeight:1.6, marginBottom:"28px" }}>A quick setup so everything — calories, protein, training — is dialed to you. Takes 30 seconds. All evidence-based.</div>
        <div style={subLabel}>YOUR NAME</div>
        <input value={d.name} onChange={e=>set("name",e.target.value)} placeholder="First name" style={inp} autoFocus />
      </div>)}

      {step===2 && (<div>
        <div style={{ fontSize:"20px", fontWeight:"700", marginBottom:"4px" }}>BODY STATS</div>
        <div style={{ fontSize:"12px", color:"#9a9a9a", marginBottom:"20px", lineHeight:1.5 }}>Used for Mifflin-St Jeor BMR — the most validated metabolic rate equation.</div>
        <div style={subLabel}>HEIGHT</div>
        <div style={{ display:"flex", gap:"8px", marginBottom:"14px" }}>
          <div style={{ flex:1, position:"relative" }}><input value={d.heightFt} onChange={e=>set("heightFt",e.target.value)} inputMode="numeric" style={inp} /><span style={{ position:"absolute", right:"12px", top:"50%", transform:"translateY(-50%)", fontSize:"11px", color:"#888" }}>ft</span></div>
          <div style={{ flex:1, position:"relative" }}><input value={d.heightIn} onChange={e=>set("heightIn",e.target.value)} inputMode="numeric" style={inp} /><span style={{ position:"absolute", right:"12px", top:"50%", transform:"translateY(-50%)", fontSize:"11px", color:"#888" }}>in</span></div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"14px" }}>
          <div><div style={subLabel}>WEIGHT (LB)</div><input value={d.weight} onChange={e=>set("weight",e.target.value)} inputMode="decimal" style={inp} /></div>
          <div><div style={subLabel}>AGE</div><input value={d.age} onChange={e=>set("age",e.target.value)} inputMode="numeric" style={inp} /></div>
        </div>
        <div style={subLabel}>BIOLOGICAL SEX (FOR BMR CALC)</div>
        <div style={{ display:"flex", gap:"8px" }}>{chip("male",d.sex,v=>set("sex",v))}{chip("female",d.sex,v=>set("sex",v))}</div>
      </div>)}

      {step===3 && (<div>
        <div style={{ fontSize:"20px", fontWeight:"700", marginBottom:"4px" }}>TRAINING</div>
        <div style={{ fontSize:"12px", color:"#9a9a9a", marginBottom:"20px", lineHeight:1.5 }}>This sets your calorie multiplier and workout intensity.</div>
        <div style={subLabel}>EXPERIENCE LEVEL</div>
        <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"16px" }}>{Object.entries(expLabel).map(([k,v])=><button key={k} onClick={()=>set("experience",k)} style={{ padding:"10px 14px", borderRadius:"3px", fontSize:"11px", letterSpacing:"1px", cursor:"pointer", fontFamily:"inherit", fontWeight:d.experience===k?"700":"500", border:d.experience===k?"1.5px solid #E8FF47":"1.5px solid #3a3a3a", background:d.experience===k?"#E8FF47":"#111", color:d.experience===k?"#0a0a0a":"#bbb" }}>{v}</button>)}</div>
        <div style={subLabel}>WEEKLY ACTIVITY</div>
        <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"16px" }}>{Object.entries(actLabel).map(([k,v])=><button key={k} onClick={()=>set("activityLevel",k)} style={{ padding:"10px 14px", borderRadius:"3px", fontSize:"11px", letterSpacing:"1px", cursor:"pointer", fontFamily:"inherit", fontWeight:d.activityLevel===k?"700":"500", border:d.activityLevel===k?"1.5px solid #47C4FF":"1.5px solid #3a3a3a", background:d.activityLevel===k?"#47C4FF":"#111", color:d.activityLevel===k?"#0a0a0a":"#bbb" }}>{v}</button>)}</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
          <div><div style={subLabel}>BAR WEIGHT (LB)</div><input value={d.barWeight} onChange={e=>set("barWeight",e.target.value)} inputMode="numeric" style={inp} /></div>
          <div><div style={subLabel}>SLEEP NEED (HRS)</div><input value={d.sleepNeed} onChange={e=>set("sleepNeed",e.target.value)} inputMode="decimal" style={inp} /></div>
        </div>
      </div>)}

      {step===4 && (<div>
        <div style={{ fontSize:"20px", fontWeight:"700", marginBottom:"4px" }}>YOUR GOAL</div>
        <div style={{ fontSize:"12px", color:"#9a9a9a", marginBottom:"20px", lineHeight:1.5 }}>This shapes your calorie target, protein ratio, and training emphasis.</div>
        <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
          {Object.entries(goalLabel).map(([k,v])=>{
            const desc = { muscle:"Caloric surplus + high protein. Prioritize progressive overload.", cut:"Caloric deficit + highest protein to preserve muscle. Controlled volume.", recomp:"Maintenance calories + high protein. Build muscle while leaning out.", endurance:"Slight surplus + moderate protein. Fuel your miles.", hybrid:"Balanced surplus + high protein. Lift heavy, run far, recover smart." }[k];
            return (<button key={k} onClick={()=>set("goal",k)} style={{ padding:"14px 16px", borderRadius:"4px", textAlign:"left", cursor:"pointer", fontFamily:"inherit", border:d.goal===k?"1.5px solid #E8FF47":"1.5px solid #2a2a2a", background:d.goal===k?"#1a1a00":"#111" }}>
              <div style={{ fontSize:"14px", fontWeight:"700", color:d.goal===k?"#E8FF47":"#e4e4e4", marginBottom:"4px" }}>{v}</div>
      ...
      {step===5 && targets && (<div>
        <div style={{ fontSize:"20px", fontWeight:"700", marginBottom:"4px" }}>YOUR PLAN</div>
        <div style={{ fontSize:"12px", color:"#9a9a9a", marginBottom:"16px", lineHeight:1.5 }}>Calculated from your stats. Adjust anything before starting — you can always change these in ⚙ Settings later.</div>
        <Card accent="#47C4FF" style={{ marginBottom:"12px" }}>
          <Label color="#47C4FF">METABOLIC BASE</Label>
          <div style={{ display:"flex", gap:"10px", marginBottom:"6px" }}>
            <div style={{ flex:1, textAlign:"center" }}><div style={{ fontSize:"9px", color:"#888", letterSpacing:"1px" }}>BMR</div><div style={{ fontSize:"18px", fontWeight:"700", color:"#fafafa" }}>{bmr}</div></div>
            <div style={{ flex:1, textAlign:"center" }}><div style={{ fontSize:"9px", color:"#888", letterSpacing:"1px" }}>TDEE</div><div style={{ fontSize:"18px", fontWeight:"700", color:"#fafafa" }}>{tdee}</div></div>
            <div style={{ flex:1, textAlign:"center" }}><div style={{ fontSize:"9px", color:"#888", letterSpacing:"1px" }}>TARGET</div><div style={{ fontSize:"18px", fontWeight:"700", color:"#E8FF47" }}>{t.cal}</div></div>
          </div>
          <div style={{ fontSize:"10px", color:"#777", lineHeight:1.5 }}>Mifflin-St Jeor BMR × {({ sedentary:"1.2", light:"1.375", moderate:"1.55", active:"1.725", very_active:"1.9" })[d.activityLevel]} activity{d.goal==="muscle"?" + 350 surplus":d.goal==="cut"?" − 500 deficit":d.goal==="recomp"?" − 100 slight deficit":d.goal==="endurance"?" + 200 fuel":""}</div>
        </Card>
        <Card accent="#4ade80" style={{ marginBottom:"12px" }}>
          <Label color="#4ade80">DAILY TARGETS (EDITABLE)</Label>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
            <div><div style={subLabel}>PROTEIN (G)</div><input value={t.protein} onChange={e=>tSet("protein",e.target.value)} inputMode="numeric" style={inp} /></div>
            <div><div style={subLabel}>CARBS (G)</div><input value={t.carbs} onChange={e=>tSet("carbs",e.target.value)} inputMode="numeric" style={inp} /></div>
            <div><div style={subLabel}>FAT (G)</div><input value={t.fat} onChange={e=>tSet("fat",e.target.value)} inputMode="numeric" style={inp} /></div>
            <div><div style={subLabel}>WATER (OZ)</div><input value={t.water} onChange={e=>tSet("water",e.target.value)} inputMode="numeric" style={inp} /></div>
          </div>
          <div style={{ fontSize:"10px", color:"#777", marginTop:"8px", lineHeight:1.5 }}>Protein: {m.ppl}g/lb — optimal range for {d.goal==="cut"?"preserving muscle in a deficit":"muscle protein synthesis"} (Morton et al. 2018). Fat: ~27% of cals for hormonal health. Carbs: remainder — fuels training.</div>
        </Card>
        <Card accent="#a3e635" style={{ marginBottom:"12px" }}>
          <Label color="#a3e635">TRAINING APPROACH</Label>
          <div style={{ fontSize:"13px", color:"#e0e0e0", lineHeight:1.6 }}>Push / Pull / Legs split — trains each muscle 2×/week with compound-first programming and progressive overload (Schoenfeld et al. 2016). Volume and intensity auto-adjust daily based on your recovery score and subjective feel.</div>
        </Card>
      </div>)}

      <div style={{ display:"flex", gap:"10px", marginTop:"24px" }}>
        {step>1 && <Btn onClick={()=>setStep(step-1)} style={{ flex:1 }}>← BACK</Btn>}
        {step<5 && <Btn primary onClick={()=>setStep(step+1)} style={{ flex:1 }}>{step===1?"LET'S GO →":"NEXT →"}</Btn>}
        {step===5 && <Btn primary onClick={finish} style={{ flex:1, padding:"16px", letterSpacing:"3px" }}>START TRAINING →</Btn>}
      </div>
    </div>
  );
}
...
.fix-safe-top { padding-top: env(safe-area-inset-top, 0px); }
.fix-safe-bottom::after { content: ''; display: block; height: env(safe-area-inset-bottom, 0px); flex-shrink: 0; }
.safe-sticky { top: env(safe-area-inset-top, 0px) !important; }
...
export default function DailyCoach() {
  ...
  async function handleOnboardingComplete(newSettings) {
  ...
  if (!settings.onboardingDone) {
    return (
      <div style={{ background:"#0a0a0a", minHeight:"100vh", fontFamily:FONT, color:"#fff", maxWidth:"480px", margin:"0 auto" }}>
        <style>{GLOBAL_CSS}</style>
        <OnboardingFlow settings={settings} onComplete={handleOnboardingComplete} />
      </div>
    );
  }
  ...
  return (
    <div style={{ background:"#0a0a0a", minHeight:"100vh", fontFamily:FONT, color:"#fff", maxWidth:"480px", margin:"0 auto", position:"relative" }}>
      ...
      {day?.plan && (
        <div style={{ padding:"10px 20px", borderBottom:"1px solid #1a1a1a", background:"#0d0d0d" }}>
          <div style={{ fontSize:"12px", color:"#b8b8b8", fontStyle:"italic", lineHeight:1.5 }}>"{quote.q}"</div>
          <div style={{ fontSize:"11px", color:"#E8FF47", marginTop:"3px" }}>— {quote.a}</div>
        </div>
      )}
